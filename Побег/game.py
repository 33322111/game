...
def load_image(n, color_key=None):
    fn = os.path.join('data', n)
    try:
...
screen.fill(back)
pg.display.update()
play = load_image("player.png")
plx = width / 2
ply = height / 2
gamer = pg.Rect(plx, ply, play.get_rect().width, play.get_rect().height)
pg.mixer.music.load(os.path.join('data', "babax_noise.mp3"))  # добавим музыку
win = 2
complication = "Normal"
best = [0, 0, 0]  # счет
lives = 1
running = True
while running:
    blue_ball = load_image("blue_ball.png")
    ...
    red_ball = load_image("red_ball.png")
    ...
    green_ball = load_image("green_ball.png")
    ...
    while ending and flag:
        ...
        best = best[:3]
        ch = text_phrase.render(f"Рекорд: {str(best[0])}", True, "black")
